...
$paramId = $_SESSION["id"];
$rsAll1 = $pdo->query("SELECT * FROM users");
$total_pages1 = ceil($rsAll1->rowCount() / $numItens1);
$rs1 = $pdo->query("SELECT * FROM users LIMIT $shift1, $numItens1;");

if(isset($_GET['admin'])){
    $adminValue = $pdo->prepare("SELECT cargo, id FROM users WHERE id = ?");
    $updateAdminValue = $pdo->prepare("UPDATE users SET cargo = ? where id = ?");

    $adminValue->execute(array($_GET['admin']));
    $setAdminLine = $adminValue->fetch(PDO::FETCH_OBJ);
    if($setAdminLine->id != $_SESSION["id"]){
        if($setAdminLine->cargo == 'USER') $updateAdminValue->execute(array('ADM', $_GET['admin']));
        else $updateAdminValue->execute(array('USER', $_GET['admin']));
    }
            ...
            <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4" style="margin-top: 5vh;">
                ...
                <div class="table-responsive">
                    ...
                    while ($row1 = $rs1->fetch(PDO::FETCH_OBJ)) {
                        ...
                        echo "<td>" . $row1->cargo . "</td>";
                        echo "<td>" . $row1->pontos . "</td>";
                        echo "<td>" . $row1->data . "</td>";
                        if($row1->cargo == 'ADM' && $row1->id != $_SESSION["id"]) echo "<td>" . "<a href=\"?admin=$row1->id\" class=\"\">Remover Admin</a>" . "</td>";
                        else if ($row1->cargo == 'USER')echo "<td>" . "<a href=\"?admin=$row1->id\" class=\"\">Tornar Admin</a>" . "</td>";
                        else echo "<td>------</td>";
                        echo "</tr>";
                    }
                    ?>
