...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['file1']) && $_FILES['file1']['error'] == 0) {
        $arquivo_tmp1 = $_FILES['file1']['tmp_name'];
        $nome1 = $_FILES['file1']['name'];

        $extensao1 = pathinfo($nome, PATHINFO_EXTENSION);

        $extensao1 = strtolower($extensao1);

        if (strstr('.jpg;.jpeg;.gif;.png', $extensao1)) {
            $novoNome1 = uniqid(time()) . '.' . $extensao1;

            $destino1 = 'images/submited/' . $novoNome1;

            if (move_uploaded_file($arquivo_tmp1, $destino1)) {
            } else {
                $file_err1 = "Erro ao salvar o arquivo.";
            }
        } else {
            $file_err1 = "Você poderá enviar apenas arquivos .jpg;*.jpeg;*.gif;*.png";
        }
    } else {
        $file_err1 = "Você não enviou nenhum arquivo!";
    }
...
// Verificar existencia de erros antes de inserir na base de dados    
    if (empty($nomeErr) && empty($stockErr) && empty($corErr) && empty($armazenamentoErr) && empty($precoErr) && empty($categoriaErr)) {
